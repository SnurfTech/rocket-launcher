...
public class RocketItemRenderer implements SpecialModelRenderer<ItemStack> {
	@SubscribeEvent
	public static void registerItemRenderers(RegisterSpecialModelRendererEvent event) {
		event.register(ResourceLocation.parse("rocket_launcher:rocket"), RocketItemRenderer.Unbaked.MAP_CODEC);
	}

	private static final Map<Integer, Function<EntityModelSet, RocketItemRenderer>> MODELS = Map
			.ofEntries(Map.entry(-1, modelSet -> new RocketItemRenderer(new ModelRocket(modelSet.bakeLayer(ModelRocket.LAYER_LOCATION)), ResourceLocation.parse("rocket_launcher:textures/item/rocket_item_texture.png"))));
	private final EntityModel<LivingEntityRenderState> model;
	private final ResourceLocation texture;
	private final LivingEntityRenderState renderState;
	private final long start;

	private RocketItemRenderer(EntityModel<LivingEntityRenderState> model, ResourceLocation texture) {
		this.model = model;
	...
	public record Unbaked(int index) implements SpecialModelRenderer.Unbaked {
		public static final MapCodec<RocketItemRenderer.Unbaked> MAP_CODEC = RecordCodecBuilder
				.mapCodec(instance -> instance.group(ExtraCodecs.NON_NEGATIVE_INT.optionalFieldOf("index").xmap(opt -> opt.orElse(-1), i -> i == -1 ? Optional.empty() : Optional.of(i)).forGetter(RocketItemRenderer.Unbaked::index)).apply(instance,
						RocketItemRenderer.Unbaked::new));
		...
		public SpecialModelRenderer<?> bake(EntityModelSet modelSet) {
			return RocketItemRenderer.MODELS.get(index).apply(modelSet);
		}
	}
}